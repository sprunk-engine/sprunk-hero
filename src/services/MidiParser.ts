...
export class MidiParser implements Parser{
    public async parseTrack(file: string): Promise<Chart> {
        ...
        return {
            song: manifest.song,
            modes: await Promise.all(manifest.modes.map(async (mode: { difficulty: string, midiPath: string }) => {
                return {
    ...
    private async parseMidi(midiTrack: string) : Promise<Note[]>{
        ...
        const guitarTrack = midi.tracks.find(track => track.name === "PART GUITAR");
